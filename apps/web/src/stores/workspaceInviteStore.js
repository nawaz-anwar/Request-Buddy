...
export const useWorkspaceInviteStore = create((set, get) => ({
  ...
  sendInvitation: async (workspaceId, workspaceName, inviteeEmail, role, inviterUserId, inviterEmail) => {
    ...
    try {
      ...
      console.log('InviteStore: Invitation created with ID:', docRef.id)
      ...
      try {
        ...
        const sendInvite = httpsCallable(functions, 'sendWorkspaceInvitation')
        ...
        if (emailResult.data.success) {
          ...
          console.warn('⚠️ Invitation created but email failed:', emailResult.data.error)
          toast.success(`Invitation created for ${inviteeEmail}`, {
