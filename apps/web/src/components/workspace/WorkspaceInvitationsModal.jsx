...
export default function WorkspaceInvitationsModal({ isOpen, onClose }) {
  ...
  const [inviterEmails, setInviterEmails] = useState({})

  // Fetch inviter emails for all pending invites
  useEffect(() => {
    const fetchInviterEmails = async () => {
      const emails = {}
      
      for (const invite of pendingInvites) {
        // If inviterEmail is already in the invite, use it
        if (invite.inviterEmail && invite.inviterEmail !== 'unknown@example.com') {
          emails[invite.id] = invite.inviterEmail
          continue
        }
        ...
        if (invite.invitedBy) {
          try {
            const userDoc = await getDoc(doc(db, 'users', invite.invitedBy))
            if (userDoc.exists()) {
              emails[invite.id] = userDoc.data().email || 'Unknown user'
            } else {
              emails[invite.id] = 'Unknown user'
            }
          } catch (error) {
            console.error('Failed to fetch inviter email:', error)
            emails[invite.id] = 'Unknown user'
          }
        } else {
          emails[invite.id] = 'Unknown user'
                          ...
                          <div className="flex items-center space-x-2 mb-2">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-gray-500 dark:text-gray-400">
                                Invited by {inviterEmails[invite.id] || invite.inviterEmail || 'Unknown user'}
                              </p>
                            </div>
                          </div>
