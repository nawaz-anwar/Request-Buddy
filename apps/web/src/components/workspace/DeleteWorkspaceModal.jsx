...
export default function DeleteWorkspaceModal({ isOpen, workspace, onClose }) {
  const { workspaces, deleteWorkspace, setCurrentWorkspace } = useWorkspaceStore()
  const { cleanup: cleanupCollections } = useCollectionStore()
  const { cleanup: cleanupRequests } = useRequestStore()
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      console.log('Deleting workspace and all related data:', workspace.id)
      
      // Delete all requests in this workspace
      ...
      const collectionDeletePromises = collectionsSnapshot.docs.map(doc => deleteDoc(doc.ref))
      await Promise.all(collectionDeletePromises)
      console.log(`Deleted ${collectionsSnapshot.docs.length} collections`)
  ...
  const handleKeyDown = (e) => {
    if (e.key === 'Escape' && !deleting) {
      handleClose()
    } else if (e.key === 'Enter' && confirmText === workspace?.name) {
      handleDelete()
    }
  }
          ...
          <div className="mb-6">
            <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4 mb-4">
              ...
              <ul className="text-sm text-red-700 dark:text-red-300 space-y-1">
                <li>• The workspace "{workspace.name}"</li>
                <li>• All collections in this workspace</li>
                <li>• All folders and requests</li>
            ...
            <div className="mb-2">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                Workspace name: <span className="font-semibold">{workspace.name}</span>
              </label>
              <input
                type="text"
                value={confirmText}
